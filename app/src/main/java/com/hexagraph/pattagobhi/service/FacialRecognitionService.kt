...
import com.hexagraph.pattagobhi.model.ProcessedImage
import com.hexagraph.pattagobhi.util.AIIntegration
import com.hexagraph.pattagobhi.util.AIIntegration.FACE_NET_EMBEDDING_SIZE
import com.hexagraph.pattagobhi.util.AIIntegration.FACE_NET_IMAGE_SIZE
import com.hexagraph.pattagobhi.util.AIIntegration.calculateCosineSimilarity
import com.hexagraph.pattagobhi.util.AIIntegration.faceNetInterceptor
import com.hexagraph.pattagobhi.util.AIIntegration.isRunning
import com.hexagraph.pattagobhi.util.AIIntegration.mobileNetInterceptor
import com.hexagraph.pattagobhi.util.AIIntegration.preprocessBitmapForMobileFaceNet
import java.nio.ByteBuffer
import java.nio.ByteOrder
import javax.inject.Inject

class FaceRecognitionService @Inject constructor() {
    fun mobileNet(face: ProcessedImage, context: Context): Result<Float> = runCatching {
    ...
    fun recognizeFace(
        ...
        return try {
            ...
            val referenceOutputBuffer = ByteBuffer.allocateDirect(FACE_NET_EMBEDDING_SIZE * 4).apply { order(
            ...
            for (data in faces) {
                ...
                val testOutputBuffer = ByteBuffer.allocateDirect(FACE_NET_EMBEDDING_SIZE * 4).apply { order(
                    ByteOrder.nativeOrder()) }
